...
# Define a generic exception message prefix for configuration errors
exception_msg = "Config.py incorrect parameterized: "

# ConfigFederated class defines configuration parameters for the federated learning simulation
class ConfigFederated():
    # Flag to enable debug mode for detailed logging and output
    debug = False
    # Number of times the entire simulation will be repeated (for averaging results)
    repeats = 1
    
    # Boolean flag to save trained model weights to disk after training
    save = False
    # Specifies which round's saved models should be loaded from disk for resuming training
    load_round = 40                        
    # Boolean flag to load previously saved models from disk
    load = False
    # Boolean flag to delete loaded models from disk after loading
    delete_on_load = False
    # Path to directory where models will be saved/loaded
    path = "./.saved/"

    # Total number of clients in the simulation (includes 1 global model + n local models)
    clients = 4                            
    # Number of rounds (iterations) of federated training to perform
    rounds = 5                             
    # Round number at which out-of-distribution (OOD) detection should be activated
    ood_round = 3
    # Number of local clients that participate in each training round (should be clients - 1)
    participants = 3                       
    # Client ID of the global/server model (usually 0)
    host_id = 0
    # List mapping each client to the dataset indices they use for training. Length must equal number of clients
    client_to_dataset = [[0, 1, 2], [0], [1], [2]]   
