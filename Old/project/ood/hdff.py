...
# Import partial function for creating function closures
from functools import partial
# Import tqdm for progress bars during computation
from tqdm import tqdm

# Import VSA helper class for hyperdimensional computing operations
...
# Define the HDFF (Hyperdimensional Feature Fusion) class for OOD detection
class Hdff():
    # ========================================================================
    # Conceptual role:
    ...
    def __init__(self, ood_config : ConfigOod, dataset_config : ConfigDataset):
        ...
        """
        # Create VSA helper instance for hyperdimensional operations (e.g., bundling, similarity)
        self.vsa = Vsa(debug=ood_config.hdc_debug)
        # Store dataset configuration for input shape information
        self.dataset_config = dataset_config
        # Store OOD configuration for hyperdimensional size and debug settings
        self.ood_config = ood_config
         
        # Initialize list to store projection matrices (one per layer)
        self.proj = []
        # Initialize list to store feature tensors extracted from model layers
        self.features = [0]
        # Initialize list to store bundled results
        self.results = []
        
        # Create dummy input tensor with correct batch_size and input_shape for feature extraction
        input_shape = (self.dataset_config.batch_size,) + self.dataset_config.input_shape
        # Initialize dummy input with ones (actual values don't matter, only shape)
