...
# Import random for random sampling of clients
import random
# Import copy for creating deep copies of models
import copy
...
# Import configuration classes for federated learning, OOD, and model settings
from config import ConfigDataset, ConfigFederated, ConfigOod, ConfigPlot
# Import Dataset class for handling data
from dataset.dataset import Dataset
# Import federated learning math utilities (like FedAvg aggregation)
from federated.math import federated_math

# Import HDFF (Hyperdimensional Feature Fusion) for OOD detection
from ood.hdff import Hdff
# Import Model class for neural networks
from model.model import Model


# Define the Federated learning environment class for Phase 1 training
class Federated:
    """
    Federated learning environment implementation.
    Phase 1 (Task 2): Initialize models + datasets, distribute weights (regression),
    train local models, aggregate using FedAvg, evaluate global model, save & plot results.
    """
    ...
    def __init__(
        ...
        """
        Initialize the Federated learning environment with configurations and datasets.
        
        Parameters:
            dataset: Dataset object containing all training data
            model: Template Model object (architecture will be cloned for each client)
            federated_config: Configuration for federated learning simulation
            ood_config: Configuration for out-of-distribution detection
            dataset_config: Configuration for dataset handling
            plot_config: Configuration for plotting results
        """
        # Store the dataset manager
        self.dataset = dataset
        # Store the template model (architecture blueprint to be cloned for each client)
        self.init_model = model  
        # Store federated learning configuration parameters
        self.federated_config = federated_config
        # Store OOD detection configuration parameters
        self.ood_config = ood_config
        # Store dataset configuration parameters
        self.dataset_config = dataset_config
        # Store plotting configuration parameters
        self.plot_config = plot_config

        # Dictionary to store model instances: client_id -> Model object (created in initialize_)
        self.models: dict[int, Model] = {}
        # Dictionary to store client data: client_id -> (train_data, val_data, test_data) tuple
        self.client_data: dict[int, tuple] = {}  

        # List to track global model loss across all training rounds
        self.global_loss_hist: list[float] = []
        # List to track global model accuracy across all training rounds
        self.global_acc_hist: list[float] = []

        # Dictionary to track OOD similarity scores for each client across rounds
        self.similarity_hist: dict[int, list[float]] = {}
        # Cache for projection matrices used in OOD detection (reused across rounds)
        self._ood_proj = None
